	...
	*/
	...
	if (r->flags == 0)
	{
		...
		while (p)
		{
			if (((p->pi) == pi) && (changed & (1 << (p->gpio))))
			{
				...
				{
					if (p->ex)
						(p->f)(pi, p->gpio, l, r->tick, p->user);
					else
						(p->f)(pi, p->gpio, l, r->tick);
				}
			}
			p = p->next;
		}
	...
	else
	{
		...
		while (p)
		{
			...
			{
				if (p->ex)
					(p->f)(pi, g, PI_TIMEOUT, r->tick, p->user);
				else
					(p->f)(pi, g, PI_TIMEOUT, r->tick);
			}
			p = p->next;
		}
	}
...
static int recvMax(int pi, void* buf, int bufsize, int sent)
{
	...
	if (count)
		recv(gPigCommand[pi], buf, count, MSG_WAITALL);
	...
	if (gPigCommand[pi] >= 0)
	{
		...
		if (gPigNotify[pi] >= 0)
		{
			...
			*userdata = pi;
			...
			if (gPthNotify[pi])
				return pi;
			return pigif_notify_failed;
		}
		return gPigNotify[pi];
	}
