...
wfRx_t wfRx[PI_MAX_USER_GPIO+1];
...
volatile uint32_t alertBits   = 0;
volatile uint32_t monitorBits = 0;
volatile uint32_t notifyBits  = 0;
volatile uint32_t scriptBits  = 0;
volatile uint32_t gFilterBits = 0;
volatile uint32_t nFilterBits = 0;
volatile uint32_t wdogBits    = 0;

static volatile int runState = PI_STARTING;

static int pthAlertRunning  = 0;
extern int pthFifoRunning;
extern int pthSocketRunning;

static gpioAlert_t      gpioAlert  [PI_MAX_USER_GPIO+1];

static gpioISR_t        gpioISR    [PI_MAX_USER_GPIO+1];

gpioGetSamples_t gpioGetSamples;

static gpioInfo_t       gpioInfo   [PI_MAX_GPIO+1];

gpioNotify_t     gpioNotify [PI_NOTIFY_SLOTS];
...
extern pthread_t pthFifo;
extern pthread_t pthSocket;
...
char *myBuf2Str(unsigned count, char *buf)
{
   static char str[128];
   int i, c;
...
void myGpioSetMode(unsigned gpio, unsigned mode)
{
   int reg, shift;
...
void myGpioWrite(unsigned gpio, unsigned level)
{
...
void myGpioSleep(int seconds, int micros)
{
...
uint32_t myGpioDelay(uint32_t micros)
{
   uint32_t start;
...
int initInitialise(void)
{
   ...
   if (!gpioMaskSet)
   {
       if (rev == 0)
           ...
           gpioMask = PI_DEFAULT_UPDATE_MASK_R1;
       else if (rev < 16)
           gpioMask = PI_DEFAULT_UPDATE_MASK_R2;
       else
           gpioMask = PI_DEFAULT_UPDATE_MASK_R3;
...
static void stopHardwarePWM(void)
{
   ...
   for (i=0; i<= PI_MAX_GPIO; i++)
   {
      if (gpioInfo[i].is == GPIO_HW_PWM)
      {
         ...
         if (pwm == 0)
             pwmReg[PWM_CTL] &= (~PWM_CTL_PWEN1);
         else
             pwmReg[PWM_CTL] &= (~PWM_CTL_PWEN2);
...
int gpioWaveAddGeneric(unsigned numPulses, gpioPulse_t *pulses)
{
   ...
   if (!pulses)
       SOFT_ERROR(PI_BAD_POINTER, "bad (NULL) pulses pointer");
   ...
   unsigned spiBits)
{
   ...
   */
   ...
   if (spi->clk_us > spi->ss_us)
       wf[2][p].usDelay = spi->clk_us;
   else
       wf[2][p].usDelay = spi->ss_us;
...
int gpioWaveDelete(unsigned wave_id)
{
   ...
   if (wave_id == (waveOutCount-1))
   {
      ...
      while ((wave_id > 0) && (waveInfo[wave_id-1].deleted))
          --wave_id;
   ...
   uint32_t next)
{
   ...
   while (count && (b<blocks))
   {
      ...
      if (count)
          nxt = chainGetCntVal(counter, (b*(blklen+1))+blklen);
      else
          nxt = waveCbPOadr(botCB);
